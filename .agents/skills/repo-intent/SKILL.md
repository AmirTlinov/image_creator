...
Build a thin MCP server that turns image-generation APIs into one stable local artifact contract: prompt in, file path out, with profile-first model choice.
...
- the agent should prefer curated profiles over raw model ids
...
1. keep curated profiles and docs aligned with live provider reality
